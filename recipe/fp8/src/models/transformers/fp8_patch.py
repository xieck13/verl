...
def per_token_cast_to_fp8(x: torch.Tensor, dtype: str="e4m3") -> Tuple[torch.Tensor, torch.Tensor]:
    BLOCK_N = 128
    M, N = x.shape
    if dtype == 'e4m3':
        y = torch.empty(M, N, device=x.device, dtype=torch.float8_e4m3fn)
    elif dtype == 'e5m2':
        y = torch.empty(M, N, device=x.device, dtype=torch.float8_e5m2)
    else:
...
def per_block_cast_to_fp8(x: torch.Tensor, dtype: str='e4m3') -> Tuple[torch.Tensor, torch.Tensor]:
    BLOCK_M = 128
    BLOCK_N = 128
    M, N = x.shape
    A, B = triton.cdiv(M, BLOCK_M), triton.cdiv(N, BLOCK_N)
    if dtype == 'e4m3':
        y = torch.empty(A * BLOCK_M, B * BLOCK_N, device=x.device, dtype=torch.float8_e4m3fn)
    elif dtype == 'e5m2':
        y = torch.empty(A * BLOCK_M, B * BLOCK_N, device=x.device, dtype=torch.float8_e5m2)
    else:
        raise RuntimeError(f"Unsupported dtype: {dtype}")
...
def deep_matmul(a, b, out=None, dtype='e4m3'):
    """
    ...
    a = pad_8(a)
    a_fp8 = per_token_cast_to_fp8(a, dtype=dtype)
    b_fp8 = per_block_cast_to_fp8(b, dtype=dtype)
    if out is None:
...
class _DeepLinear(torch.autograd.Function):
    ...
    def backward(ctx, grad_outputs):
        ...
        dweight = deep_matmul(grad_outputs.T, inp.T, dtype='e5m2')
        if inp.requires_grad:
            dinp = deep_matmul(grad_outputs, weight.T, dtype='e5m2').view(ctx.inp_shape)
